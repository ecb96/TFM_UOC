   ...
   tabsetPanel(
     tabPanel(
       ...
       checkboxGroupInput("het", label = "Selecciona tipo de heterocigoto, si procede",
                          ...
                          choiceValues = c("h", "hc")),
       h5("Si se ha escogido 'Autosómica dominante heredada'"),
       selectInput("afect", label="Indica qué parental esta afectado",
                   choices = c("Padre" = "papa", "Madre" = "mama")),
...
observeEvent(input$lectura,{
   ...
   genotype<-character()
   dp<- numeric()
   datos$format<- as.character(datos$format)
   format_split<-strsplit(datos$format[1],":")
   for (posicion in 1:length(format_split[[1]])) {
     if (format_split[[1]][posicion] == "DP"){dp_pos<- posicion}
   }
   for (i in 1:nrow(datos)){
     gt_split<-strsplit(datos$gt[i],":")
     genotype[i]<-gt_split[[1]][1]
     dp[i]<-gt_split[[1]][dp_pos]
   }  
   ...
   for (i in 1:nrow(datos)){
     ...
     for (j in length(EFF_split)){
       eff_split<- strsplit(EFF_split[[1]][1], "\\(")
       impacto[i]<- eff_split[[1]][2]
       consecuencia[i]<- eff_split[[1]][1]
     }
   }       
     ...
     if (is.null(input$padre) == FALSE & 
       ...
       if(input$origen == "her"){
         if (input$afect == "papa"){
           ...
           rs_hijo_padre_ini <- intersect(rs_hijo, rs_padre)
           rs_hijo_padre <- setdiff(rs_hijo_padre_ini, rs_madre)
           ...
           for (i in 1:length(rs_hijo_padre)){
             fila<-subset(datos_hijo, datos_hijo$ID == rs_hijo_padre[i])
             hijo_variantes<- rbind(hijo_variantes, fila)
           }
         }
         if (input$afect == "mama"){
           ...
           rs_madre<- datos_madre$ID[nfilas_madre]
           rs_padre<- datos_padre$ID[nfilas_padre]
           rs_hijo_madre_ini <- intersect(rs_hijo, rs_madre)
           rs_hijo_madre <- setdiff(rs_hijo_madre_ini, rs_padre)
           for (i in 1:length(rs_hijo_padre)){
